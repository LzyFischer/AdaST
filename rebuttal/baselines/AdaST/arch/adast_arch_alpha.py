...
class CombineCorrelationAggregation(nn.Module):
    def __init__(self, model_dim: int, alpha: float = 0.1, reduction: int = 4):
        """
        Args:
            alpha: Power applied to correlation features in gate modulation.
                   g_tilde = g * C^alpha  (paper eq. 19)
                   alpha=0.0 -> no correlation modulation (gate_net only)
                   alpha=0.1 -> paper default
                   alpha=1.0 -> linear correlation weighting
        """
        super().__init__()
        self.alpha = alpha
    ...
    def forward(self, x, x_t, x_s):
        ...
        # Apply alpha: g_tilde = g * C^alpha (paper eq. 19)
        if self.alpha != 0.0:
...
def aggregation_in_model(aggregation_type='simple_avg', model_dim=128, alpha=0.1):
    if aggregation_type == 'combine_correlation':
        return CombineCorrelationAggregation(model_dim=model_dim, alpha=alpha)
    else:
...
class AdaST(nn.Module):
    def __init__(
        ...
        alpha: float = 0.1,  # correlation modulation strength (paper eq. 19)
    ):
        super().__init__()
    ...
    def forward(self, history_data: torch.Tensor, future_data: torch.Tensor, batch_seen: int, epoch: int, train: bool, 
        ...
        if return_embeddings:
            return {
                ...
                'corr_features': corr_features.detach()  # (B, 3)
            }
